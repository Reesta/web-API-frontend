...
import { getAdminTrailAction } from "@/lib/actions/admin/admin-trail-action";
import { getCurrentUserAction } from "@/lib/actions/auth-action";

export default async function EditAdminTrailPage({
  params,
}: {
  ...
  const result = await getAdminTrailAction(id);
      ...
      <section className="relative mx-auto max-w-6xl">
        ...
        <div className="mt-6 rounded-[1.75rem] border border-white/10 bg-white/[0.04] p-6 shadow-2xl shadow-black/40 backdrop-blur sm:p-8">
          <div className="flex items-start gap-4">
            ...
            <div>
              <p className="text-xs font-black uppercase tracking-[0.3em] text-[#e9a127]">Route editor</p>
              <h1 className="mt-3 text-4xl font-black tracking-tight">Edit trail</h1>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-400">
                Update this trail&apos;s image, route details, and waypoint timeline.
              </p>
            </div>
          </div>
          <div className="mt-7"><TrailForm key={result.data.id} trail={result.data} /></div>
        </div>
      </section>
    </main>
  );
