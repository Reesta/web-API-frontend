...
import { getAdminReviewsAction } from "@/lib/actions/admin/admin-review-action";
...
export default async function AdminReviewsPage({
  ...
  const result = await getAdminReviewsAction({ page, limit, search });
    ...
    <main className="relative min-h-screen overflow-hidden bg-[#060914] px-5 py-8 text-white sm:px-8 lg:py-10">
      ...
      <div className="relative mx-auto max-w-7xl">
        {result.success && result.data && result.meta ? (
          <ReviewTable data={result.data} meta={result.meta} search={search} />
        ) : (
          <div className="rounded-[2rem] border border-red-400/30 bg-red-400/10 p-8 text-red-100">
            <h1 className="text-3xl font-black">Could not load reviews</h1>
